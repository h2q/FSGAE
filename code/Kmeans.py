...
def kmeans_onmi(edge_path, hidden_emb,comms_path,dataset_name):
    ...
    os.remove(pred_path)
    return nmi,ari,eq
def unhash_cog_kmeans_onmi(edge_path, hidden_emb,comms_path,dataset_name):
    real_path = comms_path
    g = nx.read_edgelist(edge_path, nodetype=int)
    my_seq=list(g.nodes())
    nodes=len(my_seq)
    ...
    with open(real_path, 'r') as f:
        ...
        real_com = np.array(real_com)
    n_clusters=cnt-1
    sort_myseq=sorted(list(my_seq))
    data=torch.zeros_like(hidden_emb)
    for node_id in sort_myseq:
        index1 = sort_myseq.index(node_id)
        index2 = list(my_seq).index(node_id)
        data[index2] = hidden_emb[index1]
    data=data.detach().cpu().numpy()
    kmeans = KMeans(n_clusters=n_clusters, verbose=0,random_state=7)
